...
export default function Home({ preview = false }: { preview?: boolean }) {
 ...
 const startRun=()=>{if(!catalogue.length||!selected.length)return;playMenuSound();setLeaderboardOpen(false);setSubmissionId(crypto.randomUUID());setSubmissionOutcome(null);updateRun(createRun(catalogue,selected,difficulty as Difficulty));setRunId(id=>id+1);window.scrollTo(0,0);};
 ...
 if(run)return <main className="shell page-shell"><div className="utility-row"><SoundToggle enabled={soundEnabled} onToggle={toggleSound}/></div>{leaderboard}<div hidden={leaderboardOpen}><GameBoard key={runId} run={run} onGuess={onGuess} onGiveUp={()=>{playMenuSound();updateRun(giveUp(runRef.current!,performance.now()));}} onReplay={startRun} onSettings={mainMenu} onUiSound={playMenuSound} onLeaderboard={leaderboardCategory(run.generationIds)?openLeaderboard:undefined} submitted={!!submissionOutcome}/></div></main>;
 ...
 return <main className="shell page-shell"><div className="utility-row"><SoundToggle enabled={soundEnabled} onToggle={toggleSound}/></div><section className="welcome"><section className="setup-panel">{error&&<p role="alert">{error}</p>}<div className="section-label"><h2>Gotta name 'em all!</h2></div><div className="generation-options">{generations.map((g,i)=><label key={g.id} className={'generation-card '+(selected.includes(g.id)?'selected':'')}><input className="choice-input" type="checkbox" aria-label={'Generation '+g.id} checked={selected.includes(g.id)} onChange={e=>{playMenuSound();setSelected(prev=>e.target.checked?[...prev,g.id]:prev.filter(id=>id!==g.id));}}/><span className="region-ball" aria-hidden="true"><img src={selected.includes(g.id)?'/ui/pokeball-open.png':'/ui/pokeball-closed.png'} alt=""/></span><div className="generation-art">{menuArt[artTheme][i].map(id=><img key={id} src={`/menu/${artTheme}/gen${g.id}/${id}.png`} alt="" width="64" height="64"/>)}</div><h3>{g.name}</h3><p>#{g.range}</p></label>)}</div><div className="difficulty-options" role="radiogroup" aria-label="Difficulty">{difficulties.map(d=><label className={'difficulty-card '+(difficulty===d.id?'selected':'')} key={d.id}><input className="choice-input" type="radio" name="difficulty" value={d.id} checked={difficulty===d.id} onChange={()=>{if(d.id!==difficulty)playMenuSound();setDifficulty(d.id);}}/><span className="difficulty-ball" aria-hidden="true"><img src={difficulty===d.id?'/ui/pokeball-open.png':'/ui/pokeball-closed.png'} alt=""/></span><div><strong>{d.name}</strong><small>{d.detail}</small></div></label>)}</div><div className="start-row"><div className="start-copy"><strong>{count} Pokémon</strong><span>{count?"A world of dreams and adventures with POKéMON awaits! Let's go!":'You need to select a region to guess. PROF. OAK, next door is looking for you.'}</span>{selected.length>0&&!leaderboardCategory(selected)&&<span className="leaderboard-eligibility">Note: These region selections don't have a leaderboard.</span>}</div><div className="start-buttons"><button className="primary-button red-action" onClick={openLeaderboard}>View Leaderboard</button><button className="primary-button" onClick={startRun} disabled={!count||!catalogue.length}>Play</button></div></div></section></section></main>;
}
